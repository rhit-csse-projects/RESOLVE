 ...
 */
package edu.clemson.rsrg.nProver.utilities.treewalkers;
...
public abstract class AbstractRegisterSequent extends TreeWalkerStackVisitor {
    ...
    public void postInfixExp(InfixExp exp) {
        ...
        if (!myExpLabels.containsKey(exp.getOperatorAsString())) {
            ...
            myMappings.add(exp.getOperatorAsString());
            myNextLabel++;
        }
    }
    ...
    protected int resolveArg(Exp arg) {
        if (arg instanceof ClusterExp) {
            return ((ClusterExp) arg).getClusterAccessor();
        }
        try {
