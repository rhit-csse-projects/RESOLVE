...
/**
 * <p>
 * This is the class for all the mathematical infix expression objects that the compiler builds using the ANTLR4 AST
 * nodes.
 * </p>
 *
 * @version 2.0
...
public class InfixExp extends AbstractFunctionExp {
     ...
     * @param left
     *            A {@link Exp} representing the left hand side.
     * @param opQual
     *            A {@link PosSymbol} representing the operator's qualifier.
     * @param opName
     *            A {@link PosSymbol} representing the operator.
     * @param right
     *            A {@link Exp} representing the right hand side.
     */
    public InfixExp(Location l, Exp left, PosSymbol opQual, PosSymbol opName, Exp right) {
        super(l, opQual);
        myLeftHandSide = left;
    ...
    public final Exp compareWithAssumptions(Exp exp) {
        ...
        if (this.equals(exp)) {
            ...
            retExp = new InfixExp(cloneLocation(), newLeftSide, newOpQualifier, myOperationName.clone(), newRightSide);
        } else {
            retExp = this.clone();
        }
    ...
    public boolean equivalent(Exp e) {
        ...
        if (retval) {
            ...
            retval = posSymbolEquivalent(myOperationName, eAsInfix.myOperationName)
    ...
    protected Exp copy() {
        ...
        return new InfixExp(cloneLocation(), myLeftHandSide, newOpQualifier, myOperationName.clone(),
    ...
    protected Exp substituteChildren(Map<Exp, Exp> substitutions) {
        ...
        return new InfixExp(cloneLocation(), substitute(myLeftHandSide, substitutions), newOpQualifier,
