...
/**
 * <p>
 * This is the abstract base class for all the mathematical function expression objects that the compiler builds using
...
public abstract class AbstractFunctionExp extends MathExp {
    ...
    public boolean equals(Object o) {
        ...
        if (myQualifier != null ? !myQualifier.equals(that.myQualifier) : that.myQualifier != null)
            return false;
    ...
    /**
     * <p>
     * This class represents function <em>applications</em>. The type of a function application is the type of the range
     * of the function. Often we'd like to think about the type of the <em>function itself</em>, not the type of the
     * result of its application. Unfortunately our AST does not consider that the 'function' part of a FunctionExp (as
     * distinct from its parameters) might be a first-class citizen with a type of its own. This method emulates
     * retrieving the (not actually extant) first-class function part and guessing its type. In this case, the guess is
     * "conservative", in that we guess the smallest set that can't be contradicted by the available information. For
     * nodes without a true, first-class function to consult (which, at the moment, is all of them), this means that for
     * the formal parameter types, we'll guess the types of the actual parameters, and for the return type we'll guess
     * <strong>Empty_Set</strong> (since we have no information about how the return value is used.) This guarantees
     * that the type we return will be a subset of the actual type of the function the RESOLVE programmer intends
    ...
    /**
     * <p>
     * Syntactically, a function application may have multiple parameters. Mathematically, we must be able to refer to
     * the single mathematical object that represents "all the parameters." This method returns that object.
     * </p>
     */
    /*
     ...
     * </p>
     *
     * @return An {@link SymbolTableEntry.Quantification} object representing the quantification for this expression.
     */
    ...
    /**
     * <p>
     * This method sets {@code t} as the type for an expression and it's sub-expressions.
     * </p>
     *
     * @param e
     *            The expression we wish to change the type.
     * @param t
