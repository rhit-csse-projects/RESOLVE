...
import edu.clemson.rsrg.absyn.statements.CallStmt;
...
import edu.clemson.rsrg.typeandpopulate.exception.NoneProvidedException;
...
public class JavaTranslator extends AbstractTranslator {
    ...
    /**
     * <p>
     * These are string templates used to generate code for {@link FacilityDec FacilitiDec(s)} and its associated
     * enhancements.
     * </p>
     */
    ...
    /**
     * <p>
     * A {@link ModuleParameterization} corresponding to the {@link EnhancementSpecRealizItem} being walked.
     * </p>
     */
    ...
    /**
     * <p>
     * A mapping between the {@link ModuleArgumentItem ModuleArgumentItems} representing the actual arguments of a
     * {@link FacilityDec} and their formal {@link ModuleParameterDec} bound counterparts.
     * </p>
     */
    ...
    /**
     * <p>
     * This set keeps track of the names of any {@link OperationDec OperationDec(s)} that parameterized the current
     * module.
     * </p>
     */
     ...
     * </p>
     */
    private static final String FLAG_DESC_TRANSLATE = "Translate RESOLVE code to Java.";
     ...
     * </p>
     */
    private static final String FLAG_DESC_TRANSLATE_CLEAN = "Regenerates Java code for all supporting RESOLVE files.";
    ...
    /**
     * <p>
     * The main translator flag. Tells the compiler convert {@code RESOLVE} source code to {@code Java} source code.
     * </p>
     */
    public static final Flag JAVA_FLAG_TRANSLATE = new Flag(FLAG_SECTION_NAME, "javaTranslate", FLAG_DESC_TRANSLATE);

    /**
     * <p>
     * Tells the compiler to regenerate {@code Java} code for all supporting {@code RESOLVE} source files.
     * </p>
     */
    public static final Flag JAVA_FLAG_TRANSLATE_CLEAN = new Flag(FLAG_SECTION_NAME, "javaTranslateClean",
    ...
    /**
     * <p>
     * This creates an object that overrides methods to translate a {@link ModuleDec} into a {@code Java} source file.
     * </p>
     *
     * @param builder
     ...
     */
    public JavaTranslator(MathSymbolTableBuilder builder, CompileEnvironment compileEnvironment) {
        super(builder, compileEnvironment, new STGroupFile("templates/Java.stg"));
    ...
    public final void postModuleDec(ModuleDec dec) {
        ...
        if (myActiveTemplates.size() != 1) {
            throw new IllegalStateException("Translation template stack " + "corrupted. Check your 'post' methods.");
        }
    ...
    public final void preConceptModuleDec(ConceptModuleDec dec) {
        ...
        ST concept = mySTGroup.getInstanceOf("interface_class").add("name", dec.getName().getName()).add("extend",
                "RESOLVE_INTERFACE");
    ...
    public final void preConceptRealizModuleDec(ConceptRealizModuleDec dec) {
        ...
        ST conceptBody = mySTGroup.getInstanceOf("concept_body_class").add("name", dec.getName().getName())
        ...
        for (ProgramParameterEntry p : formals) {
            addParameterTemplate(dec.getLocation(), p.getDeclaredType(), p.getName());
        }
    }
    ...
    public final void postConceptRealizModuleDec(ConceptRealizModuleDec dec) {
        for (ProgramParameterEntry p : getModuleFormalParameters(dec.getConceptName())) {
            String name = (p.getDeclaredType() instanceof PTElement) ? "getType" + p.getName() : "get" + p.getName();
            ST result = getOperationLikeTemplate(p.getDeclaredType(), name, true);

            myActiveTemplates.peek().add("functions",
                    result.add("stmts", mySTGroup.getInstanceOf("return_stmt").add("name", p.getName())));
        }
    }
    ...
    public final void preEnhancementModuleDec(EnhancementModuleDec dec) {
        ...
        ST enhancement = mySTGroup.getInstanceOf("interface_class").add("name", dec.getName().getName()).add("extend",
     ...
     */
    @Override
    public final void preEnhancementRealizModuleDec(EnhancementRealizModuleDec dec) {
        addPackageTemplate(dec);
        addReflectionImportTemplates(dec);

        List<ProgramParameterEntry> formals = getModuleFormalParameters(dec.getConceptName());

        ST enhancementBody = mySTGroup.getInstanceOf("enhancement_body_class").add("name", dec.getName().getName())
    ...
    /**
     * <p>
     * Code that gets executed after visiting an {@link EnhancementRealizModuleDec}.
     * </p>
     *
     * @param dec
     *            An enhancement realization module declaration.
     */
    @Override
    public final void postEnhancementRealizModuleDec(EnhancementRealizModuleDec dec) {
        /*
         * This is where we give the enhancement body all the functionality defined in the base concept. This is done
         * via a set of functions that share the signatures of the functions defined in the base concept, but whose
         * bodies merely call the <em>real</em> method.
         */
        try {
            ModuleScope conceptScope = myBuilder.getModuleScope(new ModuleIdentifier(dec.getConceptName().getName()));

            List<OperationEntry> conceptOperations = conceptScope.query(new EntryTypeQuery<>(OperationEntry.class,
                    MathSymbolTable.ImportStrategy.IMPORT_NONE, MathSymbolTable.FacilityStrategy.FACILITY_IGNORE));

            List<TypeFamilyEntry> conceptTypes = conceptScope.query(new EntryTypeQuery<>(TypeFamilyEntry.class,
                    MathSymbolTable.ImportStrategy.IMPORT_NONE, MathSymbolTable.FacilityStrategy.FACILITY_IGNORE));

            for (OperationEntry o : conceptOperations) {
                PTType returnType = (o.getReturnType() instanceof PTVoid) ? null : o.getReturnType();

                addEnhancementConceptualFunction(o.getDefiningElement().getLocation(), returnType, o.getName(),
                        o.getParameters());
            }

            for (ProgramParameterEntry p : getModuleFormalParameters(dec.getConceptName())) {

                addEnhancementConceptualFunction(p.getDefiningElement().getLocation(), p.getDeclaredType(),
                        (p.getDeclaredType() instanceof PTElement) ? "getType" + p.getName() : "get" + p.getName(),
                        null);
            }

            for (TypeFamilyEntry e : conceptTypes) {
                addEnhancementConceptualFunction(e.getDefiningElement().getLocation(), e.getProgramType(),
                        "create" + e.getName(), null);
            }
     ...
     *
     * @param dec
     *            A facility module declaration.
     */
    @Override
    public final void preFacilityModuleDec(FacilityModuleDec dec) {
        addPackageTemplate(dec);

        ST facility = mySTGroup.getInstanceOf("facility_class").add("name", dec.getName().getName());
    ...
    public final void postFacilityModuleDec(FacilityModuleDec dec) {
        ...
        List<OperationEntry> locals = myCurrentModuleScope.query(new EntryTypeQuery<>(OperationEntry.class,
                MathSymbolTable.ImportStrategy.IMPORT_NONE, MathSymbolTable.FacilityStrategy.FACILITY_IGNORE));
        ...
        if (invocationName == null && buildingJar) {
            throw new SourceErrorException(
                    "Cannot find the operation 'Main' in facility file " + dec.getName().getName(), dec.getLocation());
        }
    ...
    public final void preModuleParameterDec(ModuleParameterDec dec) {
        if (dec.getWrappedDec() instanceof OperationDec) {
            ...
            ST parameter = mySTGroup.getInstanceOf("parameter").add("type", dec.getName().getName()).add("name",
                    dec.getName().getName() + "Param");

            ST operationInterface = mySTGroup.getInstanceOf("interface_class").add("name", dec.getName().getName());
    ...
    public final void postModuleParameterDec(ModuleParameterDec dec) {
        if (dec.getWrappedDec() instanceof OperationDec) {
            ...
            emitDebug(dec.getLocation(), "Adding operation parameter: " + dec.getName());
        }
    }
    ...
    public final void postConceptTypeParamDec(ConceptTypeParamDec param) {
        try {
            ProgramParameterEntry ppe = myCurrentModuleScope
                    .queryForOne(new NameAndEntryTypeQuery<>(null, param.getName(), ProgramParameterEntry.class,
            ...
            ST getter = getOperationLikeTemplate(ppe.getDeclaredType(), "getType" + param.getName().getName(), false);
    ...
    public final boolean walkConstantParamDec(ConstantParamDec dec) {
        ...
        boolean translatingBody = (myCurrentModuleScope.getDefiningElement() instanceof ConceptRealizModuleDec)
        ...
        ST getter = getOperationLikeTemplate(type, "get" + name, translatingBody);
        getter.add("stmts", mySTGroup.getInstanceOf("return_stmt").add("name", name));

        myActiveTemplates.peek().add("functions", getter);

        emitDebug(dec.getLocation(), "Adding constant parameter: " + dec.getName());
    ...
    public final void preProgramVariableNameExp(ProgramVariableNameExp exp) {
        ...
        if (myCurrentModuleScope.getDefiningElement() instanceof ConceptRealizModuleDec) {
            ConceptRealizModuleDec thisModule = ((ConceptRealizModuleDec) myCurrentModuleScope.getDefiningElement());

            List<ProgramParameterEntry> formals = getModuleFormalParameters(thisModule.getConceptName());
        ...
        nameExp = (nonLocal) ? nameExp.add("name", "get" + exp.getName().getName() + "()")
    ...
    public final void preFacilityDec(FacilityDec dec) {
        ...
        try {
            myCurrentFacilityEntry = scopeToSearch
            ...
            ModuleParameterization spec = myCurrentFacilityEntry.getFacility().getSpecification();

            ModuleParameterization realiz = myCurrentFacilityEntry.getFacility().getRealization();
    ...
    public final void postFacilityDec(FacilityDec dec) {
        ...
        List<String> pathPieces = getFile(dec.getConceptName().getName()).getPkgList();
        ...
        if (dec.getEnhancementRealizPairs().size() == 1) {
            facilityType = dec.getEnhancementRealizPairs().get(0).getEnhancementName().getName();
        }

        // Create a new facility variable. This includes a "createProxy" call if needed.
        ST facilityVariable = mySTGroup.getInstanceOf("var_decl").add("type", facilityType)
                .add("name", dec.getName().getName()).add("init", myActiveTemplates.pop());

        myActiveTemplates.peek().add("variables", facilityVariable);

        myDynamicImports.add(mySTGroup.getInstanceOf("include").add("directories", pathPieces).render());

        // Debugging message
        emitDebug(dec.getLocation(), "Translated facility declaration: " + dec.getName());
    }
    ...
    public final void preEnhancementSpecRealizItem(EnhancementSpecRealizItem item) {
        ...
        List<ModuleParameterization> enhancements = myCurrentFacilityEntry.getEnhancements();
        ...
        if (myBaseInstantiation.getAttribute("arguments") instanceof ST) {
            ...
            args = new LinkedList<Object>((List) myBaseInstantiation.getAttribute("arguments"));
        }

        for (ModuleParameterization m : enhancements) {
            if (m.getModuleIdentifier().toString().equals(item.getEnhancementName().getName())) {
    ...
    public final void postEnhancementSpecRealizItem(EnhancementSpecRealizItem item) {
        ...
        String firstBodyName = myCurrentFacilityEntry.getEnhancementRealization(first).getModuleIdentifier().toString();

        String lastBodyName = myCurrentFacilityEntry.getEnhancementRealization(last).getModuleIdentifier().toString();

        if (curName.equals(lastBodyName)) {
            myActiveTemplates.peek().add("arguments", myBaseInstantiation.render());
        }
     ...
     *
     * @param item
     *            A module argument used in instantiating a {@link FacilityDec}.
     *
     * @return {@code true}
     */
    @Override
    public final boolean walkModuleArgumentItem(ModuleArgumentItem item) {
        ...
        if (wrappedDec instanceof OperationDec) {
            ProgramVariableNameExp operationName = (ProgramVariableNameExp) argumentExp;
            ST argItem = getOperationArgItemTemplate((OperationDec) myFacilityBindings.get(item).getWrappedDec(),
        ...
        // Case 3: The wrapped declaration is a concept type.
            ...
            if (type instanceof PTGeneric) {
                ProgramVariableNameExp typeName = (ProgramVariableNameExp) item.getArgumentExp();
            ...
            // Case 3.2: A facility declared type that doesn't have a concept model
                ...
                myActiveTemplates.peek().add("arguments", "new " + getTypeName(type) + "()");
            }
     ...
     *
     * @param stmt
     *            An operation call statement.
     */
    @Override
    public final void preCallStmt(CallStmt stmt) {
        ST callStmt;
        ProgramFunctionExp callingFunctionExp = stmt.getFunctionExp();
        String qualifier = getCallQualifier(callingFunctionExp.getQualifier(), callingFunctionExp.getName(),
        ...
        if (myParameterOperationNames.contains(name)) {
            callStmt = mySTGroup.getInstanceOf("qualified_call").add("name", name).add("qualifier", name + "Param");
        } else if (qualifier != null) {
            callStmt = mySTGroup.getInstanceOf("qualified_call").add("name", name).add("qualifier", qualifier);
        } else {
            callStmt = mySTGroup.getInstanceOf("unqualified_call").add("name", name);
        }
    ...
    public final void preSwapStmt(SwapStmt stmt) {
        ...
        if (stmt.getLeft().getProgramType() instanceof PTGeneric || translatingEnhancement) {
            swapStmt = mySTGroup.getInstanceOf("unqualified_call").add("name", "swap");
        } else {
            FacilityEntry definingFacility = getDefiningFacilityEntry(stmt.getLeft().getProgramType());

            swapStmt = mySTGroup.getInstanceOf("qualified_call").add("qualifier", definingFacility.getName())
                    .add("name", "swap");
        }
    ...
    public final void preTypeFamilyDec(TypeFamilyDec dec) {
        ST typeDec = mySTGroup.getInstanceOf("interface_class").add("name", dec.getName().getName())
                .add("extend", "RType").add("public", false);

        myActiveTemplates.peek().add("classes", typeDec);

        try {
            TypeFamilyEntry typeFamilyEntry = myCurrentModuleScope
                    .queryForOne(new UnqualifiedNameQuery(dec.getName().getName()))
                    .toTypeFamilyEntry(dec.getLocation());

            ST typeDefinition = getOperationLikeTemplate(typeFamilyEntry.getProgramType(),
                    "create" + dec.getName().getName(), false);

            myActiveTemplates.peek().add("functions", typeDefinition);

            emitDebug(dec.getLocation(), "Adding type family declaration: " + dec.getName());
        } catch (NoSuchSymbolException nsse) {
            noSuchSymbol(null, dec.getName());
        } catch (DuplicateSymbolException dse) {
            throw new RuntimeException(dse); // shouldn't fire.
    ...
    public final void preTypeRepresentationDec(TypeRepresentationDec dec) {
        List<SymbolTableEntry> types = myCurrentModuleScope.query(new NameQuery(null, dec.getName()));

        PTType repType = types.get(0).toProgramTypeEntry(dec.getLocation()).getProgramType();

        ST record = mySTGroup.getInstanceOf("record_class").add("name", dec.getName().getName()).add("implement",
    ...
    public final void postTypeRepresentationDec(TypeRepresentationDec dec) {
        ST instance = mySTGroup.getInstanceOf("facility_init").add("realization", dec.getName().getName());
        ...
        try {
            ProgramTypeEntry pte = myCurrentModuleScope.queryForOne(new UnqualifiedNameQuery(dec.getName().getName()))
                    .toProgramTypeEntry(dec.getLocation());

            ST returnStmt = mySTGroup.getInstanceOf("return_stmt").add("name", instance);

            ST createMethod = getOperationLikeTemplate(pte.getProgramType(), "create" + dec.getName().getName(), true)
                    .add("stmts", returnStmt);

            myActiveTemplates.peek().add("functions", createMethod);

            emitDebug(dec.getLocation(), "Adding type representation declaration: " + dec.getName());
        } catch (NoSuchSymbolException nsse) {
            noSuchSymbol(null, dec.getName());
        } catch (DuplicateSymbolException dse) {
            throw new RuntimeException(dse); // shouldn't fire.
    ...
    public final void preFacilityTypeRepresentationDec(FacilityTypeRepresentationDec dec) {
        ST record = mySTGroup.getInstanceOf("record_class").add("name", dec.getName().getName()).add("facility", true);
    ...
    public final void preParameterVarDec(ParameterVarDec dec) {
        ...
        emitDebug(dec.getLocation(), "Adding parameter variable: " + dec.getName());
    }
    ...
    protected final ST getVariableTypeTemplate(PTType type) {
        ...
        if (type instanceof PTGeneric || type instanceof PTElement) {
            result = mySTGroup.getInstanceOf("unqualified_type").add("name", "RType");
        }
        // Case 2: Program types implemented by a concept realization.
        else if (type instanceof PTRepresentation) {
            result = mySTGroup.getInstanceOf("qualified_type").add("name", getTypeName(type));

            result.add("concept", ((ConceptRealizModuleDec) currentModule).getConceptName().getName());
        }
        // Case 3: Program types declared and implemented in a facility module.
        else if (type instanceof PTFacilityRepresentation) {
            result = mySTGroup.getInstanceOf("unqualified_type").add("name", getTypeName(type));
        }
        // Case 4: Program types declared by the concept.
            ...
            if (getDefiningFacilityEntry(type) != null) {
                moduleName = getDefiningFacilityEntry(type).getFacility().getSpecification().getModuleIdentifier()
                        .toString();
            }
            ...
            // declared by the concept.
                ...
                moduleName = ((EnhancementModuleDec) myCurrentModuleScope.getDefiningElement()).getConceptName()
                        .getName();
            }
            ...
            // declared by the concept.
                ...
                moduleName = ((EnhancementRealizModuleDec) myCurrentModuleScope.getDefiningElement()).getConceptName()
                        .getName();
            }
    ...
    /**
     * <p>
     * This method is only intended to be called when translating {@link EnhancementRealizModuleDec
     ...
     *
     * @param type
     *            A {@link PTType} for the function's return type.
     * @param name
     *            The name.
     * @param parameters
     *            A list of {@link ProgramParameterEntry} representing the function's formal parameters.
     */
    private void addEnhancementConceptualFunction(Location loc, PTType type, String name,
            ImmutableList<ProgramParameterEntry> parameters) {
        ST singleLine = mySTGroup.getInstanceOf("enhanced_stmt").add("returns", type).add("name", name);
    ...
    /**
     * <p>
     * Creates and adds a formed java package template to the {@code directives} attribute of the outermost
     * {@code module} template defined in <tt>Base.stg</tt>.
     * </p>
     *
     * @param dec
     *            The {@link ModuleDec} currently being translated.
     */
    private void addPackageTemplate(ModuleDec dec) {
        List<String> pkgDirectories = getFile(dec.getName().getName()).getPkgList();
        ST pkg = mySTGroup.getInstanceOf("package").add("directories", pkgDirectories);
        myActiveTemplates.peek().add("directives", pkg);

        emitDebug(dec.getLocation(), "Adding package template for module: " + dec.getName());
    }

    /**
     * <p>
     * Creates and adds a formed java reflection package template to the {@code includes} attribute of the outermost
     * {@code module} template defined in <tt>Base.stg</tt>.
     * </p>
     *
     * @param dec
     *            The {@link EnhancementRealizModuleDec} currently being translated.
     */
    private void addReflectionImportTemplates(EnhancementRealizModuleDec dec) {
        ST imp = mySTGroup.getInstanceOf("include").add("directories", "java.lang.reflect");

        myActiveTemplates.firstElement().add("includes", imp);

        emitDebug(dec.getLocation(), "Adding reflection imports for module: " + dec.getName());
    }

    /**
     * <p>
     * Binds <em>every</em> actual parameter of a {@link FacilityDec} to its formal counterpart, as defined in a
     * {@code concept}, {@code enhancement}, or {@code realization}.
     * </p>
     *
     * @param loc
     *            The {@link Location} where we are trying to bind the facility arguments.
     * @param spec
     *            A {@link ModuleParameterization} referencing a {@code concept} or {@code enhancement}.
     * @param realiz
     *            A {@link ModuleParameterization} referencing a realization ({@code concept realization} or
     *            {@code enhancement realization}).
     */
    private void constructFacilityArgBindings(Location loc, ModuleParameterization spec,
        ...
        try {
            ...
            throw new SourceErrorException(
                    "[" + getClass().getCanonicalName() + "] " + "Error while trying to bind facility arguments.", loc);
        }
    }

    /**
     * <p>
     * This method retrieves the proper {@link ST} template for an operation as an argument.
     * </p>
     *
     * @param operation
     ...
     *            A qualifier name.
     * @param name
     *            Name of the actual operation being passed as argument.
     *
    ...
    private ST getOperationArgItemTemplate(OperationDec operation, PosSymbol qualifier, PosSymbol name) {
        ...
        int parameterNum = 0;
        ST result = mySTGroup.getInstanceOf("operation_argument_item").add("actualName", name.getName());
        ...
        try {
            OperationEntry o = myCurrentModuleScope
            ...
            for (ProgramParameterEntry p : o.getParameters()) {
                ...
                ST castedArg = mySTGroup.getInstanceOf("parameter").add("type", castedType).add("name",
                        "p" + parameterNum);
            ...
            if (myCurrentEnhancement != null) {
                realization = myCurrentFacilityEntry.getEnhancementRealization(myCurrentEnhancement)
                        .getModuleIdentifier().toString();
            } else {
                realization = myCurrentFacilityEntry.getFacility().getRealization().getModuleIdentifier().toString();
            }

            PTType returnType = (operation.getReturnTy() != null) ? operation.getReturnTy().getProgramType() : null;

            ST interior = getOperationLikeTemplate(returnType, operation.getName().getName(), true);

            myActiveTemplates.push(interior);

            for (ParameterVarDec p : operation.getParameters()) {
                addParameterTemplate(p.getLocation(), p.getTy().getProgramType(), "p" + parameterNum);
                parameterNum++;
            }

            result.add("function", myActiveTemplates.pop()).add("realization", realization).add("hasReturn",
                    returnType != null);
        } catch (NoneProvidedException npe) {
            noSuchModule(name);
        } catch (NoSuchSymbolException nsse) {
